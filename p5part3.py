...
#    def copyCons(self, NN):
    ...
    class neuron:
        ...
        def test(self, x):
            res = [ x[a]*self.War[a] for a in range( len(x) ) ]
            r = sum(res)
            if(r > 0):
                return 1
            return 0
        
        def train(self, n, x, y, f): #n = eta, x = input, y = output from neuron, f = label
            for i in range(len(x)):
                self.War[i] = self.War[i] - n*(y-f)*x[i]
    
    def process(self, x):
        HOut = [ i.test(x) for i in self.HLayer ]
        OOut = [ j.test(HOut) for j in self.OLayer ]
        return OOut.index(max(OOut))

        
    def Correct(self,x,n): #back propogation garbage, n is correction coefficient, x is input feature set
        c = 1
        for i in self.OLayer:
            HLf = [(1/(1+(math.e**(-n*j.test(x))))) for j in self.HLayer]
            f = 1/(1+(math.e**(-n * i.test(HLf))))
            HOut = [ j.test(x) for j in self.HLayer ]
            d = i.test(HOut)
            OD = ((d-f)*f*(1-f))
            for j in range(len(self.HLayer)):
                delH = f * (1-f) * (OD * i.War[j])
                for h in self.HLayer[j].War:
                    h = h + c*delH*x[j]
            for j in range(len(i.War)): #has to come after the previous for loop
                i.War[j] = i.War[j] + c*OD*x[j]
